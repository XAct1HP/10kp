...
async function orchestrate(pitch, supabase) {
  ...
  switch (fileType) {
    ...
    case "audio": return orchestrateAudio(pitch, supabase);
...
function classifyFile(pitch) {
  if (pitch.file_type === "video" || pitch.mux_asset_id) {
    // Mux-backed asset. Audio-only Mux assets share the same webhook flow
    // as video; the pipeline distinguishes them by asset metadata.
    return pitch.file_type === "audio" ? "audio-mux" : "video";
  }
...
async function orchestrateAudio(pitch) {
  const cls = classifyFile(pitch);
  if (cls === "audio-mux") {
    return orchestrateVideo(pitch); // shares transcript + Robots flow
  }
...
async function orchestrateVideo(pitch, supabase) {
  ...
  // 2) Transcript retrieval + moderation.
  let transcriptResult = null;
  let transcriptRecord = { status: pitch.transcript_status, text: pitch.transcript };
  ...
  if (pitch.transcript_status !== TRANSCRIPT_STATUS.READY &&
      pitch.transcript_status !== TRANSCRIPT_STATUS.NOT_APPLICABLE) {
    const t = await fetchMuxTranscript(pitch.mux_asset_id, pitch.mux_playback_id);
    transcriptRecord = { status: t.status, text: t.text, language: t.language, error: t.error };
    await supabase.from("pitches").update({
      transcript: t.text || null,
      transcript_status: t.status,
      transcript_language: t.language || null,
      transcript_last_error: t.error || null,
    }).eq("id", pitch.id);
  }
  if (transcriptRecord.status === TRANSCRIPT_STATUS.READY && transcriptRecord.text) {
    transcriptResult = await moderateTextWithUmgpt({
      text: transcriptRecord.text,
      kind: "transcript",
    });
  } else if (transcriptRecord.status === TRANSCRIPT_STATUS.NOT_APPLICABLE) {
    // Legitimate no-speech video. Not required — the visual channel decides.
    transcriptResult = {
      decision: "approved",
  ...
  } else if (transcriptRecord.status === TRANSCRIPT_STATUS.PROCESSING) {
    // Captions still being generated — leave transcriptResult null so the
    // combiner returns `processing`, and schedule a retry.
    transcriptResult = null;
  } else {
    // failed / errored path — hold for review rather than approving.
    transcriptResult = {
      decision: "needs_review",
      summary: `Transcript unavailable (${transcriptRecord.error || transcriptRecord.status}). Held for human review.`,
      categories: [],
      guidebookViolations: [],
      provider: PROVIDER.UMGPT_TRANSCRIPT,
      completedAt: new Date().toISOString(),
  ...
  // 3) Mux Robots visual moderation.
  ...
  if (!config.features.muxRobotsEnabled) {
    ...
    visualResult = await runOrPollVisual(pitch, supabase);
  }
...
async function runOrPollVisual(pitch, supabase) {
  ...
  if (!pitch.mux_moderation_job_id) {
    try {
      ...
      // Robots may be temporarily unavailable — return null to signal
      // "still processing", causing the outer combiner to yield processing.
...
export { MEDIA_STATUS, MODERATION_STATE };
