...
import { extractDocTextDetailed } from "./doc-extract.js";
import { MIN_PITCH_WORDS, countWords } from "../pitchWords.js";
import {
...
import { evaluateAwardEligibilityInBackground } from "../awards/eligibility.js";
import {
  MODERATION_STATE,
  TRANSCRIPT_STATUS,
  MEDIA_STATUS,
...
// ---- Minimum length ----
// Pitches must be at least MIN_PITCH_WORDS words: the typed text, the text
// inside an uploaded document, or the spoken transcript of audio/video.
// This is a hard rule, so it rejects outright instead of going through the
// combiner (which only ever holds for review unless auto-reject is on).
// It is checked before any model call, so short pitches cost nothing.
// Only content we could actually read is counted: an unreadable document
// or a transcript Mux never produced still goes to human review.
function buildMinWordsRejection(channel, wordCount, sourceLabel) {
  const summary =
    `Rejected automatically: the ${sourceLabel} has ${wordCount} word${wordCount === 1 ? "" : "s"}, ` +
    `and pitches must be at least ${MIN_PITCH_WORDS} words.`;
  const violation = { rule: `Minimum ${MIN_PITCH_WORDS} words`, explanation: summary };
  const result = {
    decision: "rejected",
    summary,
    categories: [],
    guidebookViolations: [violation],
    provider: "word-count",
    completedAt: new Date().toISOString(),
  };
  return {
    finalState: MODERATION_STATE.REJECTED,
    summary,
    components: { [channel]: { status: MODERATION_STATE.REJECTED, result } },
    flagged_categories: [],
    guidebook_violations: [violation],
    rejectionCode: "min_words",
    wordCount,
  };
}
...
async function orchestrateText(pitch) {
  const typedWords = countWords(pitch.text_content);
  if (typedWords < MIN_PITCH_WORDS) {
    return buildMinWordsRejection("text", typedWords, "pitch text");
  }
...
async function orchestrateTextDoc(pitch) {
  const doc = await extractDocTextDetailed(pitch);
  const extracted = doc.text;
  if (doc.readable) {
    const docWords = countWords(extracted);
    if (docWords < MIN_PITCH_WORDS) {
      const outcome = buildMinWordsRejection("text", docWords, "uploaded document");
...
async function orchestrateVideo(pitch, supabase) {
  ...
  do {
    if (!transcriptResult) {
      ...
      if (nextTranscript.tooShort) {
        return buildMinWordsRejection(
          "transcript",
          nextTranscript.tooShort.wordCount,
          `${classifyFile(pitch) === "video" ? "video" : "audio"} transcript`
        );
      }
    }
...
async function resolveTranscriptModeration(pitch, supabase, transcriptRecord) {
  ...
  // Minimum length: a transcript that came back (including an empty one,
  // which Mux reports as "no speech") is counted before we pay for moderation.
  if (nextRecord.status === TRANSCRIPT_STATUS.READY ||
      nextRecord.status === TRANSCRIPT_STATUS.NOT_APPLICABLE) {
    const spokenWords = countWords(nextRecord.text);
    if (spokenWords < MIN_PITCH_WORDS) {
      return { record: nextRecord, result: null, tooShort: { wordCount: spokenWords } };
    }
  }
...
async function persistTerminal(supabase, pitch, outcome) {
  ...
  // Award tracks are only decided once a pitch is actually in the gallery.
  // Deliberately NOT awaited: text pitches run this pipeline inline inside a
  // 60s request, and a second model call could push that over. The reconciler
  // cron re-runs anything still pending, so a dropped promise costs a delay,
  // not a decision.
  if (outcome.finalState === MODERATION_STATE.APPROVED) {
    evaluateAwardEligibilityInBackground(pitch.id);
  }
...
export { MEDIA_STATUS, MODERATION_STATE, classifyFile, buildMinWordsRejection };
