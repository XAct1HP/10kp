...
//
// The digests are still what the detector groups on: they are stable,
// cheap to index, and useless outside this deployment because of the
// salt. Rotating the salt invalidates every stored fingerprint at once.
//
// Since 2026-09-04 the raw address and user-agent are ALSO stored,
// alongside the digests. That reverses the original stance and the reason
// is worth writing down: during an incident the question is never "are
// these two votes from the same place?" — the hash answers that fine — it
// is "what place, and does it look like a lecture hall or a bedroom?",
// and a hash cannot answer that at all. The raw columns are admin-only
// and meant to be purged once results are final (see
// public.purge_vote_pii in migrations/20260904_vote_integrity_v2.sql).
...
export function fingerprintFromHeaders(headers) {
  ...
  return {
    ...
    // Raw, admin-only, purgeable. Truncated because a user-agent is
    // attacker-controlled text going into a table, and nothing downstream
