...
export const COACH_MODES = {
  ...
  "pitch-coach": {
    ...
    task: [
      "Coach the student's pitch draft, which should run 60 to 90 seconds spoken. They will record it as a video or audio pitch, so judge it as spoken words.",
...
const SYSTEM_PROMPT = [
  ...
  "and alumni submit short video, audio or written pitches for startup and venture ideas. A student is working through a short curriculum",
...
export function describeWorkspace(data, stepId) {
  ...
  if (include("pitch")) {
    parts.push(section("Pitch draft", PITCH_BEATS.map((b) =>
      filled(data.pitch[b.id]) ? `${b.label} (up to ~${b.seconds}s): ${clean(data.pitch[b.id])}` : ""
    )));
  }
