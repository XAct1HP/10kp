...
// An award-track or tag filter resolves, server-side, to the set of accounts
// that submitted a matching pitch. `null` means "no such filter is active" —
// which is NOT the same as an empty set, where the filter ran and matched
// nobody. Conflating the two would silently broadcast to everyone.
export function normalizeUserIdFilter(value) {
...
export function applyAccountFilters(accounts, { scope, confirmed, search, allowedUserIds } = {}) {
  ...
  const allowed = normalizeUserIdFilter(allowedUserIds);
...
export function buildBroadcastHtml(message, { subject } = {}) {
  ...
  return renderBrandedEmail({
    ...
    bodyHtml,
    footerNote:
      "You&#39;re receiving this because you have a 10,000 Pitches account. To stop receiving these emails, reply with &quot;unsubscribe&quot; in the subject.",
  });
}
...
export function buildBroadcastText(message) {
  ...
  return [
    ...
    "",
    "---",
    "You're receiving this because you have a 10,000 Pitches account.",
...
export function buildWinnerNotificationHtml({ prizeLabel, note, surveyUrl = WINNER_SURVEY_URL } = {}) {
  ...
  const bodyParagraphs = [
    // The headline above already says "Congratulations!" — repeating it here
    // read as a stutter, so the body opens straight on the news.
    `<p style="margin:0 0 18px 0;color:#374151;font-size:17px;line-height:1.7;">You&apos;ve been selected as a winner for <strong>${escapeHtml(normalizedPrizeLabel)}</strong>.</p>`,
